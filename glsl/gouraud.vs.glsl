...
vec3 computeGouraud(vec3 ambientColor, vec3 diffuseColor, vec3 specularColor, vec3 N, vec3 L, vec3 E, vec3 R)
{
...
void main() {
	...
	vec3 N = normalize(normalMatrix * normal); // eye coord
	vec3 L = normalize(mat3(viewMatrix) *  (-lightDirection) ); 
	vec3 E = normalize( -v );  // camera est à (0,0,0)
	vec3 R = normalize(reflect(-L,N)); 
	
	gouraudBlack = computeGouraud(materialAmbientColor, materialDiffuseColor, materialSpecularColor, N, L, E, R);
	 
	gouraudWhite = computeGouraud(maskLightColor, maskLightColor, maskLightColor, N, L, E, R);
